...
import account as ac            #added by PXD
import validate_mail as vmail   #added by PXD
...
def register(request, template_name=None, extra_context=None, **kwargs):#add by pxd
    ...
    if request.method=='POST': 
        ...
        exist_email = ac.exist_email_all(email)
        exist_username = ac.exist_username_all(username)
        if exist_username==1:
            return HttpResponse("already exist the username")
        if exist_email==1:
            return HttpResponse("already exist the email")
        if tenant_id!="":
            exist_tenant = ac.exist_tenant(tenant_id)
            if exist_tenant==0:
                return HttpResponse("no such a tenant")
        vmail.send_register_mail(username, password1, tenant_id, email)
        return shortcuts.render(request, 'auth/register_done.html')
    return shortcuts.render(request, 'auth/register.html')


def reset(request, template_name=None, extra_context=None, **kwargs):#add by pxd
    template_name = 'auth/reset.html'
    if request.method=='POST': 
        email=request.POST.get('email', '')
        if ac.exist_email(email)==0:
            return HttpResponse("email doesn't exist")
        res = vmail.send_reset_mail(email)
        if res == 1:
            return shortcuts.render(request, 'auth/reset_done.html')
        else:
            return HttpResponse(res)
        return HttpResponse(res)
    return shortcuts.render(request, 'auth/reset.html')

def reset_done(request, template_name=None, extra_context=None, **kwargs):#add by pxd
    return shortcuts.render(request, 'auth/reset_done.html')

def register_validate(request, token):#add by pxd
    if request.method=='GET': 
        res = ac.validate_register_url(token)
        if res==True:
            return HttpResponse("success! you can login now.")
        else:
            return HttpResponse("failure! url exipired or doesn't exist")

def reset_validate(request, token):#add by pxd
    if request.method=='GET': 
        user_id = ac.validate_reset_url(token)
        if user_id==False:
            return HttpResponse("failure! url exipired or doesn't exist")
        else:
            return HttpResponseRedirect("/horizon/auth/reset/"+user_id+"/")

def reset_password(request, user_id):#add by pxd
    template_name = 'auth/reset_password.html'
    if request.method=='POST':
        password1=request.POST.get('password1','')
        password2=request.POST.get('password2','')
        res = ac.update_password(user_id, password1)
        return HttpResponse("success, you can login now!")
    return shortcuts.render(request, 'auth/reset_password.html')
